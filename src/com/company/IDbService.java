...
// Напишите утилитарный (вспомогательный) класс DbServer, который наследует следующий интерфейс:
...
public interface IDbService extends AutoCloseable {
     ...
     * обязательные поля объекта author определены. В противном случае, метод
     * пытается обновить уже существующие записи, используя заполненные поля
     * класса для поиска подходящих записей.
     * Например, если в объекте author указан id автора,
     ...
     * обязательные поля объектов doc и author определены. В противном случае,
     * метод пытается обновить уже существующие записи, используя заполненные
     * поля объектов для поиска подходящих записей.
     *
     ...
     *                должен содержаться в заголовке или в основном тексте документа.
     *
     ...
     * удаляются и все документы, которые ссылаются на удаляемого автора.
     *
